...
### 2026-05-25 (доп.) — gap-анализ аудита ТЗ vs Beads

Сверка аудита заказчика с реальным состоянием Beads + кода. Найдены пробелы, **перепроверены по коду**:

| Пункт аудита | Факт по коду | Решение |
|---|---|---|
| ALPHA-6 AI-валидация качества фото | Только статичные tips в `Capture.tsx`, AI-оценки нет | Создана **sym-t9d** [P1] → эпик pc2 |
| BETA-3 Визуализация расшифровки | `ResultDisplay.tsx` — только текст; recharts лишь в Mood | Создана **sym-r8g** [P2] → эпик c5v |
| ALPHA-10 Ручная выдача кредитов | ✅ **РЕАЛИЗОВАНА** (`UserDetailPage.tsx` → RPC `admin_adjust_credits`, лимиты, лог `credit_transactions`) | Задача НЕ нужна |
| ALPHA-10 Управление подписками в админке | Нет, но зависит от модели тарифов | Отложено до решения по тарифам |
| ALPHA-3 Модель тарифов | Расхождение с ТЗ, ждём заказчика | Создана **sym-vgc** [P1, blocked-by-client] → эпик p4i |

Подтверждено: подписочная инфраструктура реализована (закрытый эпик **sym-mpt** + миграция **sym-9bf**), сравнение чашек / Mood / Daily Insights задачами не трекались (закрывать нечего).
