...
> ⚠️ Это тестовая карта YooKassa. Реальные деньги НЕ списываются.
...
1. Откройте [Supabase Dashboard](https://supabase.com/dashboard/project/johspxgvkbrysxhilmbg)

2. **Таблица `purchases`**:
   - Перейдите в Table Editor → purchases
   - Найдите запись с вашим `user_id`
   - Статус должен быть `succeeded`
   - Поле `yukassa_payment_id` заполнено
   - Поле `paid_at` содержит дату оплаты

3. **Таблица `user_credits`**:
   - Перейдите в Table Editor → user_credits
   - Найдите запись с вашим `user_id`
   - Кредиты должны быть начислены (например, `basic_credits: 1`)

4. **Таблица `payment_analytics`**:
   - Перейдите в Table Editor → payment_analytics
   - Должны быть записи:
     - `payment_started` — создание платежа
...
### Сценарий 2: Отклонённая карта ❌

1. Выберите любой тариф
2. Введите карту `4000 0000 0000 0002`
3. Попробуйте оплатить
4. **Ожидание**:
   - Ошибка оплаты
   - В `purchases` статус `canceled` или остаётся `pending`

### Сценарий 3: 3D Secure 🔐

1. Выберите любой тариф
2. Введите карту `4000 0000 0000 3063`
3. Оплатите
4. **Ожидание**:
...
## Дополнительные тестовые карты

| Сценарий | Номер карты | Результат |
|----------|-------------|-----------|
| Успешная оплата | `4111 1111 1111 1111` | Платёж проходит |
| Отклонено | `4000 0000 0000 0002` | Платёж отклонён |
| 3D Secure | `4000 0000 0000 3063` | Требуется 3DS |
| Недостаточно средств | `4000 0000 0000 9995` | Ошибка баланса |
