# Техническое задание: Сервис психологического анализа кофейной гущи "Symancy"

## EXECUTIVE SUMMARY

### Проект в двух словах

**Symancy** — AI-платформа психологического самопознания через анализ кофейной гущи, превращающая древнюю практику в современный инструмент персонального развития.
...
CREATE TABLE users (
    ...
    telegram\_id BIGINT UNIQUE,

    whatsapp\_id VARCHAR(50),

    wechat\_id VARCHAR(100),
    ...
    language\_code VARCHAR(10) DEFAULT 'ru',
...
CREATE TABLE subscriptions (
    ...
    plan\_type VARCHAR(20), \-- free, basic, advanced, premium
...
CREATE TABLE analyses (
    ...
    input\_type VARCHAR(20), \-- image, text

    input\_data JSONB,

    image\_url TEXT,

    patterns\_detected JSONB,

    interpretation TEXT,

    tokens\_used INTEGER,

    ai\_model VARCHAR(50),
...
CREATE TABLE usage\_limits (
    ...
    photos\_used INTEGER DEFAULT 0,

    texts\_used INTEGER DEFAULT 0,

    questions\_used INTEGER DEFAULT 0,
...
CREATE TABLE payments (
    ...
    subscription\_id UUID REFERENCES subscriptions(id),
...
CREATE TABLE pattern\_library (

    id UUID PRIMARY KEY DEFAULT gen\_random\_uuid(),

    pattern\_name VARCHAR(100),

    pattern\_category VARCHAR(50),

    description TEXT,

    psychological\_meaning TEXT,

    positive\_aspects TEXT,

    negative\_aspects TEXT,
...
rate\_limit:user:{user\_id}:photos \= 3

rate\_limit:user:{user\_id}:texts \= 50
...
const delays \= {
  ...
  typing\_start: \[2, 5\], // секунды

  between\_messages: \[3, 7\],

  analysis\_time: \[15, 30\]
...
const experiments \= {
  ...
  'onboarding\_video': {
    ...
    variants: \['control\_text', 'video\_tutorial'\],

    traffic\_allocation: 0.5,

    min\_sample\_size: 1000,

    success\_metric: 'activation\_rate'

  },

  'pricing\_test': {

    variants: \['299\_rub', '399\_rub'\],

    traffic\_allocation: 0.3,

    min\_sample\_size: 2000,

    success\_metric: 'revenue\_per\_user'
...
### 17.2. Оптимальная команда (Growth)
...
*Документ подготовлен: 16 сентября 2025* *Обновлен: 16 сентября 2025* *Head требования: Считаем себестоимости по нижней планке* *Версия: 2.0* *Статус: Готов к презентации*
