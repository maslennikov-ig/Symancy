...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                    SYMANCY PLATFORM ARCHITECTURE (Current)              │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ┌────────────────────────────────────────────────────────────────┐     │
│  │                   OMNICHANNEL INPUT LAYER                      │     │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐ │     │
│  │  │ Telegram Bot │  │ Web App      │  │ API (B2B)            │ │     │
│  │  │ • Inline mode│  │ • PWA        │  │ • RESTful            │ │     │
│  │  │ • WebApp     │  │ • Real-time  │  │ • Type-safe (tRPC)   │ │     │
│  │  └──────┬───────┘  └──────┬───────┘  └──────────┬───────────┘ │     │
│  └─────────┼─────────────────┼─────────────────────┼─────────────┘     │
│            │                 │                     │                    │
│            └─────────────────┼─────────────────────┘                    │
...
│  │  • Adaptive compression     • Format normalization             │     │
│  │  • Quality preservation     • Resolution standardization       │     │
│  │  • EXIF handling            • Color space correction           │     │
│  │                                                                │     │
│  │  Tech: browser-image-       Tech: Sharp (Node.js)              │     │
│  │        compression                                             │     │
│  └────────────────────────────────┬───────────────────────────────┘     │
│                                   ▼                                     │
│  ┌────────────────────────────────────────────────────────────────┐     │
...
│  │  │                                                         │  │     │
│  │  │  • Simultaneous visual + semantic understanding         │  │     │
│  │  │  • Multi-zone analysis (RIM / CENTER / BOTTOM)          │  │     │
│  │  │  • Pattern recognition (spirals, lines, clusters)       │  │     │
│  │  │  • Density distribution analysis                        │  │     │
│  │  │  • Symbolic interpretation based on traditions          │  │     │
│  │  └─────────────────────────────────────────────────────────┘  │     │
│  │                                                                │     │
│  │  ┌─────────────────────────────────────────────────────────┐  │     │
│  │  │  PROPRIETARY PROMPT ENGINEERING LAYER                   │  │     │
│  │  │                                                         │  │     │
│  │  │  • Domain-specific instruction set for tasseography     │  │     │
│  │  │  • Structured zone analysis prompts                     │  │     │
│  │  │  • Pattern-to-interpretation mapping                    │  │     │
│  │  │  • Multilingual output control (RU/EN/ZH)               │  │     │
│  │  │  • Consistency optimization techniques                  │  │     │
│  │  └─────────────────────────────────────────────────────────┘  │     │
│  └────────────────────────────────┬───────────────────────────────┘     │
│                                   ▼                                     │
│  ┌────────────────────────────────────────────────────────────────┐     │
│  │              AI PERSONA ENGINE (Proprietary R&D)               │     │
│  │                                                                │     │
│  │  ┌────────────────────┐    ┌────────────────────┐             │     │
│  │  │ АРИНА              │    │ КАССАНДРА          │             │     │
│  │  │ • Warm, supportive │    │ • Deep symbolic    │             │     │
│  │  │ • Positive focus   │    │ • Historical refs  │             │     │
│  │  │ • Gentle guidance  │    │ • Detailed analysis│             │     │
│  │  └────────────────────┘    └────────────────────┘             │     │
│  │                                                                │     │
│  │  Unique stylistic fine-tuning through prompt engineering       │     │
│  └────────────────────────────────┬───────────────────────────────┘     │
│                                   ▼                                     │
│  ┌────────────────────────────────────────────────────────────────┐     │
...
│  │                                                                │     │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐    │     │
│  │  │ PostgreSQL  │  │ Supabase    │  │ YooKassa            │    │     │
│  │  │ (Supabase)  │  │ Auth        │  │ Payments            │    │     │
│  │  │             │  │             │  │                     │    │     │
│  │  │ • Profiles  │  │ • Web OAuth │  │ • Credit system     │    │     │
│  │  │ • History   │  │ • Telegram  │  │ • Subscriptions     │    │     │
│  │  │ • Analytics │  │   Login     │  │ • 54-FZ compliance  │    │     │
│  │  └─────────────┘  └─────────────┘  └─────────────────────┘    │     │
│  └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                        SYMANCY VISUAL PROOF ARCHITECTURE                 │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ┌──────────────┐    ┌─────────────────────────────────────────────┐    │
│  │   КЛИЕНТ     │    │         VISUAL PROOF MODULE (CV)            │    │
│  │  (Telegram/  │───►│                                             │    │
│  │    Web)      │    │  ┌─────────────┐   ┌─────────────────────┐  │    │
│  └──────────────┘    │  │ Preprocessing│   │ Instance Segmentation│  │    │
│         │            │  │ • Нормализация│   │ • Детекция чашки    │  │    │
│         │            │  │ • Коррекция  │   │ • Маски паттернов   │  │    │
│         │            │  │   освещения  │   │ • Координаты (x,y)  │  │    │
│         │            │  └──────┬──────┘   └─────────┬───────────┘  │    │
│         │            │         │                     │              │    │
│         │            │         ▼                     ▼              │    │
│         │            │  ┌─────────────────────────────────────────┐│    │
...
│         │            │  │  • Density Mapping (тепловая карта)     ││    │
│         │            │  │  • Texture Analysis (Gabor, LBP)        ││    │
│         │            │  │  • Flow Direction (вектор потока)       ││    │
│         │            │  │  • Pattern Recognition (спираль, линия) ││    │
│         │            │  └─────────────────┬───────────────────────┘│    │
│         │            │                    │                        │    │
│         │            │                    ▼                        │    │
│         │            │  ┌─────────────────────────────────────────┐│    │
...
│         │            │  • Структурированные CV-признаки с ID       │    │
│         │            │  • Инструкцию: интерпретировать строго      │    │
│         │            │    на основе предоставленных данных         │    │
│         │            │  • Требование: ссылаться на ID признаков    │    │
│         │            │                                             │    │
│         │            └─────────────────┬───────────────────────────┘    │
│         │                              │                                │
│         │                              ▼                                │
...
│         │            │         VERIFICATION LAYER                  │    │
│         │            │                                             │    │
│         │            │  • Парсинг атрибуций в ответе LLM          │    │
│         │            │  • Валидация: каждый тег → реальный ID     │    │
│         │            │  • Отклонение необоснованных утверждений   │    │
│         │            │  • Расчёт Audit Score (% верифицированных) │    │
│         │            │                                             │    │
│         │            └─────────────────┬───────────────────────────┘    │
│         │                              │                                │
│         ▼                              ▼                                │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │                    FRONTEND OVERLAY (Visual Proof)               │  │
│  │                                                                  │  │
│  │  • Canvas/SVG отрисовка контуров поверх оригинального фото     │  │
│  │  • Интерактивная связь: клик на текст → подсветка зоны на фото │  │
│  │  • Координаты масок передаются с сервера (не генерируются)     │  │
│  │                                                                  │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
