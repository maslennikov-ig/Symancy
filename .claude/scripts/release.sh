...
update_changelog() {
    ...
    # Read existing changelog via temp file to avoid SIGPIPE with pipefail
    if [ -f "$changelog_file" ]; then
        ...
        if grep -q "## \[Unreleased\]" "$tmpfile"; then
            ...
            {
                head -n "$unreleased_line" "$tmpfile"
                echo ""
                echo "$new_entry"
                tail -n +"$((unreleased_line + 1))" "$tmpfile"
            } > "$changelog_file"
        else
...
update_release_notes() {
    ...
    # Read existing release notes via temp file to avoid SIGPIPE with pipefail
    if [ -f "$release_notes_file" ]; then
        local tmpfile
        tmpfile=$(mktemp)
        trap "rm -f '$tmpfile'" RETURN
        cp "$release_notes_file" "$tmpfile"

        # Check if file has header "# Release Notes"
        if grep -q "^# Release Notes" "$tmpfile"; then
            ...
            if [ -n "$first_release_line" ] && [ "$first_release_line" -gt 0 ]; then
                ...
                {
                    head -n "$((first_release_line - 1))" "$tmpfile"
                    echo "$new_entry"
                    echo ""
                    tail -n +"$first_release_line" "$tmpfile"
                } > "$release_notes_file"
            else
