...
run_preflight_checks() {
    ...
    if ! git diff-index --quiet HEAD -- 2>/dev/null; then
        ...
        # Count different types of changes - support BOTH .claude and .gemini paths
        local new_agents=$(echo "$file_status" | grep -E "^A.*(\.claude|\.gemini)/agents/.*\.md$" | wc -l)
        local new_skills=$(echo "$file_status" | grep -E "^A.*(\.claude|\.gemini)/skills/.*/SKILL\.md$" | wc -l)
        local new_commands=$(echo "$file_status" | grep -E "^A.*(\.claude|\.gemini)/commands/.*\.md$" | wc -l)
        local modified_agents=$(echo "$file_status" | grep -E "^M.*(\.claude|\.gemini)/agents/.*\.md$" | wc -l)
        local modified_scripts=$(echo "$file_status" | grep -E "^M.*(\.claude|\.gemini)/scripts/.*\.sh$" | wc -l)
        local modified_skills=$(echo "$file_status" | grep -E "^M.*(\.claude|\.gemini)/skills/.*/SKILL\.md$" | wc -l)
        local modified_commands=$(echo "$file_status" | grep -E "^M.*(\.claude|\.gemini)/commands/.*\.md$" | wc -l)
        local modified_docs=$(echo "$file_status" | grep "\.md$" | grep -vE "(\.claude|\.gemini)/" | wc -l)
        ...
        if [ "$new_agents" -gt 0 ]; then
            commit_type="feat"
            commit_scope="agents"
            local agent_file=$(echo "$file_status" | grep -E "^A.*(\.claude|\.gemini)/agents/.*\.md$" | safe_first | awk '{print $2}')
        ...
        # 2. New skills
            ...
            commit_type="feat"
            commit_scope="skills"
            local skill_file=$(echo "$file_status" | grep -E "^A.*(\.claude|\.gemini)/skills/.*/SKILL\.md$" | safe_first | awk '{print $2}')
        ...
        # 3. New commands
            ...
            local cmd_file=$(echo "$file_status" | grep -E "^A.*(\.claude|\.gemini)/commands/.*\.md$" | safe_first | awk '{print $2}')
